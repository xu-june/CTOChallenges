...
test_base = pd.read_csv(
    test_path, encoding="latin1", index_col=0, header=0
)  # bases are DataFrames
...
cols_to_keep = [
    ...
    "Certification Type",
    "Certification Name",
    "Is there a course with this certification?",
    "Activity",
    "Support Group",
    "Business Justification",
    "What % of the conference is business development?",
    "Travel Required",
]
...
combined_dummies = pd.get_dummies(
    data=combined_base, columns=cols_to_keep, drop_first=True
)  # Dummy values allow training algorithm to process the values - can't process strings

# Separate the training rows from the test rows
combined_train = combined_dummies.dropna(
    subset=["Category"]
)  # Train rows have a Category, so ignore the ones that don't
combined_test = combined_dummies[combined_dummies.Category.isnull()].drop(
    ["Category"], axis=1
)  # Test rows are where Category is blank, so just take those
...
train, test = train_test_split(combined_train, test_size=0.02)

# separate Category
x_train = train.drop(
    ["Category"], axis=1
)  # x is everything but Category column, y is only the categories
...
LR = LogisticRegression(
    C=8.5, solver="liblinear"
)  # Using liblinear to avoid ConvergenceWarning
LR.fit(
    x_train, y_train_preprocessed.values.ravel()
)  # Model is learning the relationship between the descriptors and the Category int values - training
predictions = LR.predict(
    x_test
)  # Use info from the training (line above) to predict Category column for x_test - still part of training
print(accuracy_score(y_test_preprocessed.values.ravel(), predictions))

predictions_test = LR.predict(
    combined_test
)  # Test for real! Predict Category column for test csv
